...
def execute_from_command_line(biomolecule, msa_file, 
    ...
    outfile_base = 'deltaphi_indsite.tx' if indsite else 'deltaphi_epistatic.txt'
    outfile = dir_output / outfile_base
    logger.info(f'\nWriting output data to file {outfile}')
    with open(outfile ,'w') as fh:
        fh.write(f'#Target sequence:{mut_inst.refseq}\n')
